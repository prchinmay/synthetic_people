...
def network_posewarp(param):
    ...
    bg_tgt_m = keras.layers.multiply([bg_tgt, bg_mask], name='bg_tgt_masked')
    y = keras.layers.add([fg_tgt, bg_tgt_m])

    #model = Model(inputs=[src_in, pose_src, pose_tgt, src_mask_prior, trans_in], outputs=[y])
    #model = Model(inputs=[src_in, pose_src, pose_tgt, src_mask_prior, trans_in], outputs=[bg_tgt, fg_tgt, bg_mask])
    model = Model(inputs=[src_in, pose_src, pose_tgt, src_mask_prior, trans_in], outputs=[y, fg_tgt, bg_mask, bg_tgt])
